...
ggsave(filename = "pbmc3k_umap.jpg", height = 8, width = 12, plot = plot, quality = 50)
```
...
# computation time
pbmc <- JackStraw(pbmc, num.replicate = 15)
pbmc <- ScoreJackStraw(pbmc, dims = 1:20)
```
...
library(ggplot2)
pdf("homework_p1.pdf")
DimPlot(pbmc, reduction = "tsne", label = TRUE, label.size = 5)+ ggtitle("tSNE projection of 68k PBMCs, where each cell is grouped into one of the 10 clusters") +theme(plot.title = element_text(size =10))
dev.off()
```
...
pdf("homework_p2_1.pdf")
for (ind in 0:(clusters-1)){
  cluster <- subset(pbmc, idents = ind)
  cluster <- FindNeighbors(cluster, dims = 1:13)
  cluster <- FindClusters(cluster, resolution = 0.12)
  pbmc$subcluster[Cells(cluster)] <- paste(ind, Idents(cluster), sep = '_')
  print(DimPlot(subset(pbmc, idents = ind), reduction = "tsne", group.by = "subcluster", label = TRUE, label.size = 5) + ggtitle(paste("tSNE projection for subcluster: ", ind)))
}
dev.off()
```
...
```{r}
pdf("homework_p2_2.pdf")
DimPlot(pbmc, group.by = "subcluster", reduction = "tsne", label = TRUE, label.size = 5) + ggtitle("overall t-SNE visualization with all clusters") +theme(plot.title = element_text(size = 12))
dev.off()
```
